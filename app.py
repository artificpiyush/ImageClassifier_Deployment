...
# Loading my model
model = load_model('CIFAR_piyush2.h5')  
...
def prepare_image(image_path):
    ...
    img = img / 255.0  
    img = np.expand_dims(img, axis=0)  # Expanding the dimensions to fit the model input (1, 32, 32, 3)
    return img
...
def upload_image():
    ...
    if file and allowed_file(file.filename):  # Only proceeds if the file has a valid extension
        try:
            ...
            percentages = [round(p * 100, 2) for p in predictions[0]]  # Conversion to Percentages
            
            class_names = ['airplane', 'automobile', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']
            os.remove(image_path)  

            # Redirecting to result page with probabilities and class names
